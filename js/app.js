...
// create function for store data
const loadProducts = () => {
  const data = [
    {
      id: 1,
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product rounded-md bg-white shadow">
      ...
      <h3 class="product-title text-4xl font-bold mb-4">${product.title}</h3>
      ...
      <p class="p-3 shadow-lg text-2xl font-bold"><i class="fas fa-users text-yellow-700"></i> ${product.rating.count}</p>
      </div>
      </div>
      </div>
      `;
